...
<body>
    <h1><title>${todo.todoNo}번 할 일 수정</title></h1>
    <hr>
    <form action="/todo/update" method="POST">
        ...
        <%-- textarea는 white-space : pre-wrap;으로 설정 되어 있어 시작, 종료 태그
             사이 공백이 모두 인식 된다. == pre태그처럼 인식됨 -> 특정 값만 작성 해놓고 싶으면,
             시작, 종료 사이에 공백 없이 작성하셈
         --%>
        <div>
            <textarea name="todoContent" cols="50" rows="5" placeholder="상세 내용">${todo.todoContent}</textarea>
        </div>

        <%-- 할 일 번호 숨겨둠(제출할때 번호까지 같이 제출하려고) --%>
